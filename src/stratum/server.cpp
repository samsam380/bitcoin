...
UniValue Server::HandleMessage(uint64_t session_id, const UniValue& request)
{
    ...
    if (method == "mining.submit") {
        ...
        if (!val.accepted_share) {
            ...
            LogPrintf("Stratum share rejected session=%u job_id=%s extranonce1=%s extranonce2=%s ntime=%s nonce=%s version_bits=%s job_version=%08x submitted_version_bits=%08x mask=%08x final_version=%08x coinbase_raw_prefix=%s coinbase_raw_size=%zu coinbase_hash=%s merkle_root=%s header80=%s block_hash=%s share_target=%s network_target=%s session_difficulty=%.8f reason=%s\n",
                      session_id, submit->job_id, session.extranonce1, submit->extranonce2, submit->ntime, submit->nonce, submit->version_bits.value_or(""), val.job_version, val.submitted_version_bits, val.version_rolling_mask, val.final_version, val.coinbase_raw_prefix, val.coinbase_raw_size, val.coinbase_hash.GetHex(), val.merkle_root.GetHex(), val.header_hex, val.block_hash.GetHex(), val.share_target.GetHex(), val.network_target.GetHex(), session.difficulty, val.reject_reason);
        ...
        LogPrintf("Stratum share accepted session=%u job_id=%s extranonce1=%s extranonce2=%s ntime=%s nonce=%s version_bits=%s job_version=%08x submitted_version_bits=%08x mask=%08x final_version=%08x coinbase_raw_prefix=%s coinbase_raw_size=%zu coinbase_hash=%s merkle_root=%s header80=%s block_hash=%s share_target=%s network_target=%s session_difficulty=%.8f accepted_block=%d\n",
